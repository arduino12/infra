...
class ATProtocol(serial.threaded.Protocol):
    ...
    def send_packet(self, packet, terminator=SEND_TERMINATOR):
        if self.transport is not None:
            self.transport.write(packet.encode(self.ENCODING) + terminator)

    def _handle_event(self, event):
        self._logger.debug('event received: %s', event)

    def send_command(self, command, response='OK', timeout=1, terminator=SEND_TERMINATOR):
        with self.response_lock:
            self._logger.debug('%s ->', command)
            self.send_packet(command, terminator)
            lines = []
            while True:
                try:
